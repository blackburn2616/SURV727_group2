...
east_gb <- c(adjacent[[5]][1], adjacent[[11]][1], adjacent[[17]][1])
print(east_gb)
#is this supposed to use xpath?
```

```{r}
#| label: areas east chr xpath
#areas <- html_elements(gb_html, xpath = '//*[@id="mw-content-text"]/div[1]/div[13]/table/tbody/tr[2]/td/div/table/tbody')
#str(areas)

#names <- html_text(areas)
#rm(names)
```
...
for (i in 2:10){
  ...
  nds <- html_elements(webpage, xpath)
  ...
  
  all_restaurants <- all_restaurants %>% rbind(df)
}
```
