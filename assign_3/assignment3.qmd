...
#find the position
html_element(gb_html, xpath = "//table[caption = 'Historical population']")
```
...
for (i in east_gb_chr) {
  url <- paste0("https://en.wikipedia.org/wiki/", i)
  webpage <- read_html(url)
  tables <- webpage |>
  html_element(xpath = "//table[caption = 'Historical population']")
  tbl2 <- html_table(tables, fill = TRUE)
  tables_all[[i]] <- tbl2 |> 
    select(Census, `Pop.`, `%±`)
}

do.call(cbind, tables_all)

print(tables_all)

#how to use cbind, what is table supposed to look like
#hist_pop <- cbind(hist_pop, tables_all$`Kenwood,_Chicago`)
```
